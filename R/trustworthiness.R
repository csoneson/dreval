...
#'
#' The trustworthiness was proposed by Kaski et al, as a local quality measure
...
#' trustworthiness measure indicates to which degree we can trust that the
#' points placed closest to a given sample in the low-dimensional representation
#' are really close to the sample also in the original data set. The \code{kTM}
...
#'
calcTrustworthinessFromDist <- function(distOriginal, distLowDim, kTM) {
    ...
    rankOriginal <- apply(as.matrix(distOriginal), 2,
                          function(w) order(order(w)))
    rankLowDim <- apply(as.matrix(distLowDim), 2,
                        function(w) order(order(w)))

    calcTrustworthinessFromRank(rankOriginal = rankOriginal,
...
#' @param kTM The number of nearest neighbors
#'
calcTrustworthinessFromRank <- function(rankOriginal, rankLowDim, kTM) {
    ...
    1 - 2/(N * kTM * (2 * N - 3 * kTM - 1)) *
        sum(vapply(seq_len(ncol(rankLowDim)), function(i) {
            sum((rankOriginal[, i] - kTM) * (rankLowDim[, i] <= kTM) *
                    (rankOriginal[, i] > kTM))
        }, NA_real_))
}

...
```{r}
dre <- dreval(sce = pbmc3ksub, refType = "assay", refAssay = "logcounts", 
              dimReds = NULL, nSamples = 1000, kTM = c(25, 100), 
...
In the example above, we used the original feature space ("logcounts" assay) as
the reference space, to which all reduced dimension representations were
...
```{r}
dre <- dreval(sce = pbmc3ksub, refType = "dimred", refDimRed = "PCA", 
              dimReds = NULL, nSamples = 1000, kTM = c(25, 100), 
              verbose = FALSE, distNorm = "l2")
dre$scores
cowplot::plot_grid(plotlist = dre$plots$disthex)
